...
*! _mm_xpose -- run-mode wrapper around xpose.  Transform events carry the
...
program define _mm_xpose
    ...
    local nin = _N
    local kin = c(k)
    capture noisily xpose `0'
    local rc = _rc
    if `rc' {
        _mm_post , dofile(`"`dof'"') line(`mml') class(transform)             ///
            cmd(xpose) subtype(`"`sub'"') keys(`"`keys'"') master(work)       ///
            result(work) nin(`nin') kin(`kin') opts(`"`jopts'"')              ///
            severity(stop)                                                    ///
            flags(`"!! xpose failed with r(`rc') -- pipeline stopped here"')
