...
*! _mm_collapse -- run-mode wrapper around collapse.  Transform events carry the
...
program define _mm_collapse
    ...
    local sub  "."
    local keys "."
    if regexm(`"`opts'"', "by\(([^)]*)\)") local keys = regexs(1)
    ...
    local nin = _N
    local kin = c(k)
    capture noisily collapse `0'
    local rc = _rc
    if `rc' {
        _mm_post , dofile(`"`dof'"') line(`mml') class(transform)             ///
            cmd(collapse) subtype(`"`sub'"') keys(`"`keys'"') master(work)       ///
            result(work) nin(`nin') kin(`kin') opts(`"`jopts'"')              ///
            severity(stop)                                                    ///
            flags(`"!! collapse failed with r(`rc') -- pipeline stopped here"')
