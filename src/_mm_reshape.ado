...
*! _mm_reshape -- run-mode wrapper around reshape.  Transform events carry the
...
program define _mm_reshape
    ...
    if `"`w1'"' != "" local sub `"`w1'"'
    local kk ""
    if regexm(`"`opts'"', "i\(([^)]*)\)") local kk = regexs(1)
    if regexm(`"`opts'"', "j\(([^)]*)\)") {
        local jspec = regexs(1)
        gettoken jv : jspec, parse(" ")
        local kk = strtrim(`"`kk' `jv'"')
    }
    if `"`kk'"' != "" local keys `"`kk'"'
    ...
    local nin = _N
    local kin = c(k)
    capture noisily reshape `0'
    local rc = _rc
    if `rc' {
        _mm_post , dofile(`"`dof'"') line(`mml') class(transform)             ///
            cmd(reshape) subtype(`"`sub'"') keys(`"`keys'"') master(work)       ///
            result(work) nin(`nin') kin(`kin') opts(`"`jopts'"')              ///
            severity(stop)                                                    ///
            flags(`"!! reshape failed with r(`rc') -- pipeline stopped here"')
