...
*! _mm_expand -- run-mode wrapper around expand.  Transform events carry the
...
program define _mm_expand
    ...
    local nin = _N
    local kin = c(k)
    capture noisily expand `0'
    local rc = _rc
    if `rc' {
        _mm_post , dofile(`"`dof'"') line(`mml') class(transform)             ///
            cmd(expand) subtype(`"`sub'"') keys(`"`keys'"') master(work)       ///
            result(work) nin(`nin') kin(`kin') opts(`"`jopts'"')              ///
            severity(stop)                                                    ///
            flags(`"!! expand failed with r(`rc') -- pipeline stopped here"')
