...
*! _mm_fillin -- run-mode wrapper around fillin.  Transform events carry the
...
program define _mm_fillin
    ...
    local nin = _N
    local kin = c(k)
    capture noisily fillin `0'
    local rc = _rc
    if `rc' {
        _mm_post , dofile(`"`dof'"') line(`mml') class(transform)             ///
            cmd(fillin) subtype(`"`sub'"') keys(`"`keys'"') master(work)       ///
            result(work) nin(`nin') kin(`kin') opts(`"`jopts'"')              ///
            severity(stop)                                                    ///
            flags(`"!! fillin failed with r(`rc') -- pipeline stopped here"')
    ...
    local sev "note"
    if `nout' > `nin' {
        local flags = trim(string(`nout' - `nin', "%20.0fc")) + " rows filled in"
    }

    _mm_post , dofile(`"`dof'"') line(`mml') class(transform) cmd(fillin)      ///
        subtype(`"`sub'"') keys(`"`keys'"') master(work) result(work)         ///
        nin(`nin') kin(`kin') nout(`nout') kout(`kout') opts(`"`jopts'"')     ///
        severity(`sev') flags(`"`flags'"')
    if "$MM_R_NOLED" == "" {
        di as txt "mergemap: fillin " as res `"`jopts'"' as txt                ///
