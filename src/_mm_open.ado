...
*! _mm_open -- open a file mergemap just wrote, using the operating system's
*! own handler.  Called from the click-to-run link that -mergemap draw- prints
...
program define _mm_open
    version 16
    gettoken f 0 : 0, parse(",")
    local f = subinstr(`"`f'"', `"""', "", .)
    local f = strtrim(`"`f'"')
    if `"`f'"' == "" local f `"$MM_LASTOUT"'
    if `"`f'"' == "" {
        di as err "mergemap: nothing to open yet"
        exit 198
    }
